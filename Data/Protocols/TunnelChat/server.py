...
class Group:
    ...
    def sendMessage(self, message, username):
        for member in self.onlineMembers:
            if member != username:
                self.clients[member].send(bytes(f"\n{username}: {message}", "utf-8"))


def tunnelchat(client, username, groupname):
    while True:
...
def handshake(client):
    ...
    if groupname in groups:
        if username in groups[groupname].allMembers:
            ...
            groups[groupname].sendMessage(username + " has requested to join the group.", "TunnelChat")
            client.send(b"/wait")
            print("Join Request:", username, "| Group:", groupname)
        threading.Thread(target=tunnelchat, args=(client, username, groupname,)).start()
    else:
...
def server_main(ip, port):
    ...
    while True:
        ...
        server = threading.Thread(target=handshake, args=(client,))
        server.daemon = True
        server.start()


def server(ip, port):
    serverThread = threading.Thread(target=server_main, args=(ip, port,))
    serverThread.daemon = True
    serverThread.start()
    allprocess.insert(1, serverThread)
